...
public class CustomTitleBar extends RelativeLayout {
    ...
    public CustomTitleBar(Context context, AttributeSet attrs) {
        ...
        if (attributes != null) {
            //处理titleBar背景色
            int titleBarBackGround = attributes.getResourceId(R.styleable.CustomTitleBar_title_background_color, R.color.white);
...
/*            //设置左边按钮的文字
            ...
            if (!TextUtils.isEmpty(leftButtonText)) {
                ...
                int leftButtonDrawable = attributes.getResourceId(R.styleable.CustomTitleBar_left_button_drawable, R.drawable.head_return_key);
                if (leftButtonDrawable != -1) {
                    titleBarLeftBtn.setBackgroundResource(leftButtonDrawable);
                }
            }*/

            int leftButtonDrawable = attributes.getResourceId(R.styleable.CustomTitleBar_left_button_drawable, R.drawable.login_btn_back);
            if (leftButtonDrawable != -1) {
                //titleBarLeftBtn.setBackgroundResource(leftButtonDrawable);
                titleBarLeftBtn.setImageResource(leftButtonDrawable);
            }
            ...
            if (titleTextDrawable != -1) {
                ...
                //标题大小
                float titleTextSize = attributes.getDimension(R.styleable.CustomTitleBar_title_text_size,-1);
                if(titleTextSize > 0) {
                    titleBarTitle.setTextSize(COMPLEX_UNIT_PX,titleTextSize);
                }

                //标题style加粗
                String titleTextStyle = attributes.getString(R.styleable.CustomTitleBar_title_text_style);
                if(titleTextStyle != null && titleTextStyle.equals("bold")) {
                    TextPaint tp = titleBarTitle.getPaint();
            ...
            if (!TextUtils.isEmpty(rightButtonText)) {
                ...
                float rightButtonTextSize = attributes.getDimension(R.styleable.CustomTitleBar_right_button_text_size,  -1);
                if(rightButtonTextSize > 0){
                    titleBarRightBtn.setTextSize(COMPLEX_UNIT_PX,rightButtonTextSize);
                }

                String rightButtonTextStyle = attributes.getString(R.styleable.CustomTitleBar_right_button_text_style);
                if(rightButtonTextStyle != null && rightButtonTextStyle.equals("bold")){
                    TextPaint tp = titleBarRightBtn.getPaint();
                    tp.setFakeBoldText(true);
                }
